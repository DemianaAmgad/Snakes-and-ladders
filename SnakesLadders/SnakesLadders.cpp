    ...
    for (int i = 0; i < 8; i++)     //Loop repeats 8 times
    {
        ...
        if (randomLoc >= 10 && randomLoc <= 98)        //Makes sure number is in range
        {
            if (board[randomLoc] != "S " && board[randomLoc] != "L ")       //Code executes if the location is not already a ladder or snake
            {
                ...
                } while (randomLoc - randomLength <= 0 || board[randomLoc - randomLength] == "S " || board[randomLoc - randomLength] == "L ");       //Continues generating a random number until the length of the snake does not go below the board size and until the ladder end is not a snake or ladder
